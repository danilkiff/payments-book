...
- Result now: 0 of 42 figures fit marginalia as generated.
- Smallest current width: `fig:mc-ipm-structure` at 103.4 mm.
- Widest current figure: `todo:ch26-payment-gateway:gateway-components` at 271.4 mm.
...
- `margin-ok`: 0
- `main-text`: 27
- `vertical-relayout-for-marginalia`: 15

## vertical-relayout-for-marginalia

| id | bbox mm | target | reason |
|---|---:|---|---|
| `fig:interchange-flow` | 166.9 x 38.6 | `marginal-vertical` | Простой линейный commission-flow из четырёх узлов можно свернуть в узкий top-down стек без потери смысла. |
| `fig:emv-chip-architecture` | 124.8 x 84.9 | `marginal-vertical` | У схемы короткие подписи компонентов и одна локальная идея; её реально перерисовать как компактный вертикальный architectural sketch. |
| `fig:emv-cryptogram-decision` | 162.7 x 89.2 | `marginal-vertical` | Это один decision chain; его естественная форма для поля — узкая вертикаль сверху вниз. |
| `fig:emv-dda-sequence` | 136.5 x 72.7 | `marginal-vertical` | Участников всего двое, а цепочку доверия можно вынести в боковой rail, поэтому narrow vertical layout реалистичен. |
| `fig:contactless-protocol-stack` | 143.1 x 61.5 | `marginal-vertical` | Стек уже концептуально вертикальный; нужен только более узкий набор подписей и более плотная упаковка слоёв. |
| `fig:token-detokenization-flow` | 176.2 x 51.9 | `marginal-vertical` | Сюжет уже доказан в репо как пригодный к marginal flow: основной поток можно сделать вертикальным, а vault вынести sidecar-узлом. |
| `fig:crypto-dukpt` | 162.4 x 109.2 | `marginal-vertical` | Деривация DUKPT — по сути одна цепочка сверху вниз; после сокращения подписей она может лечь в узкую вертикаль. |
| `fig:crypto-hsm-flow` | 208.1 x 71.4 | `marginal-vertical` | Это одна авторизационная ось с одним боковым HSM-узлом, хороший кандидат на top-down marginal flow. |
| `fig:mc-ipm-structure` | 103.4 x 71.8 | `marginal-vertical` | Структура `файл -> запись -> поля` естественно укладывается в короткий вертикальный stack. |
| `fig:mir-unionpay-routing` | 138.2 x 118.7 | `marginal-vertical` | Routing tree компактный по идее и может быть переписан как узкое decision tree сверху вниз. |
| `fig:sbp-architecture` | 159.6 x 101.9 | `marginal-vertical` | Трёхуровневая архитектура уже иерархична; её можно свести к top-down layout с краткими подписями. |
| `fig:fraud-hybrid` | 257.0 x 83.9 | `marginal-vertical` | Это pipeline с одной главной осью и парой sidecar-узлов, хороший кандидат для маргинального vertical flow. |
| `fig:pisp-flow` | 182.1 x 48.7 | `marginal-vertical` | Flow можно свернуть в top-down сценарий с одним боковым блоком банка/SCA без потери основной логики. |
| `fig:recon-exception-waterfall` | 269.0 x 99.9 | `marginal-vertical` | Waterfall по смыслу уже top-down; в узкой вертикали он будет чище, чем в wide-flow. |
| `todo:ch26-payment-gateway:gateway-components` | 271.4 x 55.8 | `marginal-vertical` | Gateway pipeline линейный; при сокращении подписей его можно сделать узкой вертикальной схемой с обратным ответом как side note. |
