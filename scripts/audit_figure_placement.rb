...
NARROW_WIDTH_MM = 45.0
NARROW_HEIGHT_MIN_MM = 60.0
NARROW_HEIGHT_MAX_MM = 80.0
NARROW_HEIGHT_HARD_MAX_MM = 96.0
PT_TO_MM = 0.352778
...
VERTICAL_RELAYOUT = {
  ...
  "fig:token-detokenization-flow" => "Сюжет уже доказан в репо как пригодный к узкому vertical flow: основной поток можно сделать вертикальным, а vault вынести sidecar-узлом.",
  "fig:crypto-hsm-flow" => "Это одна авторизационная ось с одним боковым HSM-узлом, хороший кандидат на top-down narrow flow.",
  ...
  "fig:fraud-hybrid" => "Это pipeline с одной главной осью и парой sidecar-узлов, хороший кандидат для узкого vertical flow.",
...
MAIN_TEXT_OVERRIDES = {
  "fig:four-party-model" => "Топология держится на одновременном сравнении пяти равноправных ролей; узкий формат поля исказит горизонтальные связи.",
  ...
  "fig:decline-retry-tree" => "Важны точные code groups и retry-правила; для узкого формата поля здесь слишком плотная логика и длинные листья."
}.freeze
...
def main_text_reason(entry)
  overflow = entry["layout"] == "narrow-vertical" ? "Даже после vertical relayout " : ""
  ...
  when "timeline"
    "#{overflow}временная ось теряет смысл при сжатии до узкого формата поля; фазы и подписи нужно видеть одновременно."
  when "decision-tree"
...
def narrow_ok?(entry, dims)
  entry["layout"] == "narrow-vertical" &&
    dims["width"] <= NARROW_WIDTH_MM &&
    dims["height"] <= NARROW_HEIGHT_HARD_MAX_MM
end

def recommendation_for(entry, dims)
  if narrow_ok?(entry, dims)
    {
      "recommendation" => "narrow-ok",
      "target_layout" => "narrow-vertical",
      "reason" => if dims["height"] <= NARROW_HEIGHT_MAX_MM
                    "Текущий vertical relayout уже помещается в узкий формат поля и остаётся в типовом диапазоне высоты."
                  else
                    "Текущий vertical relayout уже помещается в узкий формат поля; высота выше типовых 80 мм, но ещё приемлема для сложной схемы."
                  end
    }
  elsif VERTICAL_RELAYOUT.key?(entry["id"])
    {
      "recommendation" => "vertical-relayout-for-narrow-layout",
      "target_layout" => "narrow-vertical",
...
manifest.each do |entry|
  ...
  if VERTICAL_RELAYOUT.key?(entry["id"]) && dims["width"] <= NARROW_WIDTH_MM
    entry["layout"] = "narrow-vertical"
  end
  ...
  entry["placement_review"] = {
    "bbox_mm" => dims,
    "narrow_layout_limits_mm" => {
      "width" => NARROW_WIDTH_MM,
      "height_min" => NARROW_HEIGHT_MIN_MM,
      "height_max" => NARROW_HEIGHT_MAX_MM,
      "height_hard_max" => NARROW_HEIGHT_HARD_MAX_MM
    },
    "fits_narrow_layout_now" => narrow_ok?(entry, dims),
...
lines << ""
lines << "- Narrow-layout target: width <= #{NARROW_WIDTH_MM} mm, typical height #{NARROW_HEIGHT_MIN_MM}-#{NARROW_HEIGHT_MAX_MM} mm."
lines << "- Result now: #{manifest.count { |entry| entry.dig("placement_review", "fits_narrow_layout_now") }} of #{manifest.size} figures fit the narrow layout as generated."
...
lines << "## Summary"
lines << ""
lines << "- `narrow-ok`: #{counts.fetch("narrow-ok", 0)}"
lines << "- `main-text`: #{counts.fetch("main-text", 0)}"
lines << "- `vertical-relayout-for-narrow-layout`: #{counts.fetch("vertical-relayout-for-narrow-layout", 0)}"
lines << ""

[
  "narrow-ok",
  "vertical-relayout-for-narrow-layout",
  "main-text"
].each do |bucket|
  lines << "## #{bucket}"
  lines << ""
