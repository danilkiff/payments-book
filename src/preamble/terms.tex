...
%   \Term{label}
% Для редкой явной заметки на полях используйте:
%   \termmargin{label}
%   \termnote{label}
% Звёздочная форма сохранена для обратной совместимости:
%   \term*{label}
...
\newcommand{\termmargintext}[1]{%
  \footnotesize
  \textbf{\glsentryname{#1}}\\
  \glsentryuseri{#1}%
}

\NewDocumentCommand{\termmargin}{o m}{%
  \IfNoValueTF{#1}{%
    \marginnote{\termmargintext{#2}}%
  }{%
    \marginnote[#1]{\termmargintext{#2}}%
  }%
}

\NewDocumentCommand{\termnote}{o m}{%
  \IfNoValueTF{#1}{%
    \termmargin{#2}%
  }{%
    \termmargin[#1]{#2}%
...
{Международная организация по стандартизации; публикует отраслевые и технические стандарты.}

\DeclareTerm
{pci-dss}
{PCI DSS}
{PCI DSS}
{Стандарт защиты карточных данных.}
{Стандарт безопасности данных индустрии платёжных карт, обязательный для участников карточной инфраструктуры.}

\DeclareTerm
{rsa}
{RSA}
{RSA}
{Асимметричный криптографический алгоритм для подписей и шифрования.}
{Асимметричный криптографический алгоритм, который в платёжных системах часто используется для цифровых подписей и обмена ключами.}

\DeclareTerm
{jcb}
{JCB}
{JCB}
{Международная карточная схема японского происхождения.}
{Международная карточная схема, возникшая в Японии и работающая на глобальном рынке платежей.}
...
{Sensitive Authentication Data — чувствительные аутентификационные данные, которые стандарт PCI DSS запрещает хранить после авторизации.}
...
{БЭСП}
{БЭСП}
{Банковские электронные срочные платежи — режим валового расчёта в реальном времени в платёжной системе Банка России.}
{БЭСП — система банковских электронных срочных платежей, режим валового расчёта в реальном времени (RTGS) в платёжной системе Банка России для крупных и срочных межбанковских переводов.}

\DeclareTerm
{rtgs}
{RTGS}
{RTGS}
{Real-Time Gross Settlement — валовой расчёт в реальном времени.}
{RTGS (Real-Time Gross Settlement) — режим валового расчёта в реальном времени, при котором каждый платёж обрабатывается индивидуально и окончательно без взаимозачёта.}
