...
% ВАЖНО — Bad (канон фигур, красный «внимание»): \colorlet, чтобы цвет следовал
% за каноном. ПРАКТИКА — тон обложки CoverSteel #2F6E95 (gen_cover.py); CoverSteel
% определяется в cover.tex позже по порядку загрузки, поэтому дублируем hex здесь.
\colorlet{ImportantInk}{Bad}
\definecolor{PracticeInk}{HTML}{2F6E95}
\definecolor{MarkerSand}{HTML}{EFE4C2}  % песочный highlight — в тон открывателей частей
...
% --- Стиль таблиц (тёмная header-strip) ---
% Плашка HdrSlate (поднятый сине-стальной, мягче мёртвого Ink) на всю ширину
% строки хедера, белый PT Sans Bold в ячейках.
...
%     \bottomrule
%   \end{tabularx}
% HdrSlate — «поднятый» тёмный сине-стальной для хедера таблиц (см. комментарий выше).
\definecolor{HdrSlate}{HTML}{2C3650}
\newcommand{\headrow}{\rowcolor{HdrSlate}}
...
\DeclareRobustCommand{\highlight}[1]{%
  \begingroup
  \sethlcolor{MarkerSand}%
  \hl{#1}%
  \endgroup
}

% --- Базовый стиль notebox ---
% Мягкая плашка: бледная заливка титульной полосы (цвет!14) с тёмным цветным
% титулом (sans-serif), едва тонированное тело (цвет!4), скруглённые углы
% (arc 4pt), без рамки и линеек. Тон держится в одной гамме с заливками фигур
% (15%) и пастелью открывателей частей — блок не «давит» на полосу и читается
% как самостоятельная единица. Цвет титульной полосы/тела задаёт \colornotebox.
\tcbset{
  notebox/.style={
    enhanced,
    breakable=false,
...
% Титульная полоса — заливка цвет!14, текст титула — насыщенный цвет;
% тело — цвет!4. Скругление и отсутствие рамки берутся из стиля notebox.
